...
vi.mock('../ui/PokemonDetailsHeader', () => ({
  PokemonDetailsHeader: ({ name }: any) => (
    <div data-testid="header">{name}</div>
  ),
}));
vi.mock('../ui/PokemonDetailsBadges', () => ({
  PokemonDetailsBadges: ({ types }: any) => (
    <div data-testid="badges">{types.join(',')}</div>
  ),
}));
...
vi.mock('../ui/PokemonDetailsDescription', () => ({
  PokemonDetailsDescription: ({ description }: any) => (
...
describe('PokemonDetailsView', () => {
  ...
  it('should render error alert when error occurs', () => {
    ...
    render(<PokemonDetailsView id="1" />);
    expect(
      screen.getByText(/Failed to load Pok.mon details/i)
    ).toBeInTheDocument();
  });
